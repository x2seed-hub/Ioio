            ...
            if _G.AutoFarmAllBoss then
                pcall(function()
                    ...
                    if game:GetService("Workspace").Enemies:FindFirstChild(boss) then
                        ...
                        BossSuperFrame = game:GetService("ReplicatedStorage"):FindFirstChild(boss).HumanoidRootPart.CFrame
                    if BypassTP then
                        if (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - BossSuperFrame.Position).Magnitude > 1500 then
                            BTP(BossSuperFrame * CFrame.new(5,10,2))
                            wait(3)
                        elseif (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - BossSuperFrame.Position).Magnitude < 1500 then
                            topos(BossSuperFrame * CFrame.new(5,10,2)))
                        end
                    else
                        topos(BossSuperFrame * CFrame.new(5,10,2))
                    end
                        ...
                        for i, v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if workspace.Enemies:FindFirstChild("PirateBrigade") or workspace.Enemies:FindFirstChild("PirateGrandBrigade") or workspace.Enemies:FindFirstChild("FishBoat") then
                                repeat task.wait()
                                    ...
                                    topos(BoatCFrame * CFrame.new(0, 30, -20))
                                    Skillaimbot = true
